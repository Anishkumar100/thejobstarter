...
export default function AptitudeLesson() {
  const { lessonSlug } = useParams();
  const { lessons, fetchLessons, subtopics, loading, fetchSubtopics } = useAptitudeStore();
  ...
  if (!lesson && !loading) {
    return (
      <div className="container" style={{ paddingTop: 'var(--space-xl)' }}>
        <Link to="/aptitude" className="detail-back"><ArrowLeft01Icon size={16} /> Back to Aptitude</Link>
  ...
  return (
    <div>
      <Helmet>
        <title>{lesson?.title || 'Lesson'} — Aptitude — TheJobStarter</title>
      ...
      <section className="dsa-hero">
        ...
        <div className="container dsa-hero__content">
          <Link to="/aptitude" className="detail-back" style={{ marginBottom: 'var(--space-md)', display: 'inline-flex', alignItems: 'center', gap: '0.5rem', color: '#fff', textShadow: '1px 1px 0 #000' }}>
            <ArrowLeft01Icon size={16} /> Back to Aptitude
          </Link>
        ...
        {!loading && (
          <div className="listing-grid">
            {filtered.map((subtopic, i) => (
              ...
              >
                <Link to={`/aptitude/${lessonSlug}/${subtopic.slug}`} className="dsa-lesson-card">
                  {subtopic.image && (
