...
/* ── Source file: pass as first arg (e.g. `node generate_seed.mjs present.md`), default next.md ── */
const sourceFile = process.argv[2] || 'next.md';

/* ── Parse source markdown JSON blocks ── */
const md = fs.readFileSync(`server/dsa-content/${sourceFile}`, 'utf8');
 ...
 * Run: node dsa-content/seed_${lesson.slug}.mjs
 * NOTE: Generated from dsa-content/${sourceFile} — do not hand-edit; regenerate via generate_seed.mjs ${sourceFile} after updating ${sourceFile}.
 */
...
fs.writeFileSync(`server/dsa-content/seed_${lesson.slug}.mjs`, out.join('\n'), 'utf8');
