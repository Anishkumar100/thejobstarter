...
export default function AptitudeList() {
  const { lessons, loading, error, fetchLessons } = useAptitudeStore();
  ...
  /* Fetch hero image from homepage topic with category 'APT' */
  useEffect(() => {
    apiRequest('/topics')
      .then(res => {
        const aptTopic = (res.data || []).find(t => t.category === 'APT');
        if (aptTopic?.image) setHeroImage(aptTopic.image);
      })
      .catch(err => console.error('[APT] Topics fetch failed:', err.message));
  }, []);
  ...
  return (
    <div>
      <Helmet>
        <title>Aptitude Concepts — TheJobStarter</title>
        <meta name="description" content="Master Aptitude Concepts for placement interviews. Curated problems with detailed solutions." />
      </Helmet>
      ...
      <section className="dsa-hero">
        ...
        <div className="container dsa-hero__content">
          <h1 className="dsa-hero__title">Aptitude &amp; Reasoning</h1>
          <p className="dsa-hero__desc">
            Ace the aptitude rounds of placement interviews. From percentages, ratios,
            and number series to puzzles, syllogisms, and data interpretation — every
            topic broken down into structured lessons with curated problems and
            detailed solutions.
          </p>
              ...
              {section.lessons.map((lesson, i) => (
                ...
                >
                  {lesson.locked ? (
                    ...
                    <Link to={`/aptitude/${lesson.slug}`} className="dsa-lesson-card">
                      <div className="dsa-lesson-card__img">
                        <img src={lesson.image || `https://picsum.photos/seed/${lesson.slug}/400/200`} alt={lesson.title} />
                      </div>
                      <div className="dsa-lesson-card__body">
