...
export default function AdminAptitudeProblemList() {
  const { problems, loading, fetchProblems, deleteProblem } = useAptitudeStore();
  const [filterLesson, setFilterLesson] = useState('');
  const [filterSubtopic, setFilterSubtopic] = useState('');
  const [availableSubtopics, setAvailableSubtopics] = useState([]);
  const { lessons, fetchLessons, subtopics, fetchSubtopics } = useAptitudeStore();
  ...
  const rows = problems.map(p => ({
    ...
    actions: (
      <div className="admin-actions">
        <Link to={`/admin/aptitude/problems/${p._id}/edit`} className="btn btn--sm">Edit</Link>
  ...
  return (
    <div>
      <Helmet><title>Admin Aptitude Problems — TheJobStarter</title></Helmet>
      ...
      <div className="listing-header">
        <div>
          <h1 className="listing-header__title">Aptitude Problems</h1>
        ...
        <Link to="/admin/aptitude/problems/new" className="btn btn--primary">+ New Problem</Link>
      </div>
