...
/*
 * AdminAptitudeMeta — CRUD management for Aptitude categories, topics, and companies
 * Mirrors AdminDbmsMeta but uses /api/aptitude-meta endpoint
 */
export default function AdminAptitudeMeta() {
  ...
  const fetchItems = async () => {
    ...
    setError(null);
    try {
      const res = await apiRequest('/aptitude-meta');
      setItems(res.data || []);
    } catch (err) {
      console.error('[APT-META] Fetch failed:', err.message);
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      if (editingId) {
        await apiRequest(`/aptitude-meta/${editingId}`, { method: 'PUT', body: JSON.stringify(payload) });
        showMsg('Updated!');
      } else {
        await apiRequest('/aptitude-meta', { method: 'POST', body: JSON.stringify(payload) });
        showMsg('Created!');
      }
      setEditingId(null);
      await fetchItems();
    } catch (err) {
      console.error('[APT-META] Save failed:', err.message);
  ...
  const handleDelete = async (item) => {
    ...
    try {
      await apiRequest(`/aptitude-meta/${item._id}`, { method: 'DELETE' });
      showMsg('Deleted!');
      if (editingId === item._id) setEditingId(null);
      await fetchItems();
    } catch (err) {
      console.error('[APT-META] Delete failed:', err.message);
      showMsg('Delete failed', 'error');
    }
  };

  const handleSeed = async () => {
    if (!confirm('Replace all Aptitude categories, topics, and companies with defaults?')) return;
    setSeeding(true);
    try {
      await apiRequest('/aptitude-meta/seed', { method: 'POST' });
      showMsg('Seeded with defaults!');
      await fetchItems();
    } catch (err) {
      console.error('[APT-META] Seed failed:', err.message);
  ...
  if (loading && items.length === 0) {
    return <Loader text="LOADING APTITUDE META..." />;
  }
  ...
  return (
    <div>
      <Helmet><title>Aptitude Meta — Admin</title></Helmet>

      <div className="listing-header">
        <h1 className="listing-header__title">Aptitude Categories, Topics &amp; Companies</h1>
