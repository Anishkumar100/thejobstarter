...
import { useAptitudeMetaStore } from '../stores/useAptitudeMetaStore.js';
import { apiRequest } from '../api/client.js';
import Button from '../components/ui/Button.jsx';

export default function AdminAptitudeLessonEdit() {
  const { id } = useParams();
  const navigate = useNavigate();
  const { lessons, fetchLessons, createLesson, updateLesson } = useAptitudeStore();
  const { categories, fetchAllMeta } = useAptitudeMetaStore();
  const isNew = !id;
  ...
  /* Pick first category from meta as default for new lessons or if current is stale */
  useEffect(() => {
      ...
      reader.onload = async () => {
        try {
          const res = await apiRequest('/media/upload', {
            method: 'POST',
            body: JSON.stringify({ file: reader.result, fileName: `apt-lesson-${slug || 'new'}-${Date.now()}` })
          });
          setImage(res.url);
        } catch (err) {
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      setSaved(true);
      setTimeout(() => navigate('/admin/aptitude'), 1500);
    } catch (err) {
