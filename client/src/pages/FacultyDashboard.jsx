...
import {
  Users, Layers, FileText, AlertCircle, GraduationCap, Building2, TrendingUp, Download,
  BarChart3, Activity, Zap, ChevronLeft, ChevronRight, ArrowRight, CheckCircle,
  UserPlus, Eye
} from 'lucide-react';
...
/*
 * FacultyDashboard — Dashboard for a faculty member.
 * Mirrors CoordinatorDashboard section-for-section (stats grid, charts,
 * batch cards, recently joined, needs attention, top performers, quick
 * actions + export) but every endpoint and drill-down is scoped to the
 * faculty member's own batches.
 */
export default function FacultyDashboard() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      console.error('[FACULTY DASH] Error:', err.message);
  ...
  useEffect(() => {
    ...
    const fetchAll = async () => {
      try {
        const res = await apiRequest('/faculty/students');
        ...
        console.error('[FACULTY DASH] Error fetching students:', err.message);
      }
  ...
  /* ── CSV Export (batch-level, client-side) ── */
  const exportCSV = () => {
    ...
    push(['THEWEYTES FACULTY DASHBOARD EXPORT']);
    push([`Centre: ${center?.name || 'N/A'}`, `Exported: ${new Date().toLocaleString()}`]);
    push([]);
    push(['COLUMN EXPLANATIONS']);
    push(['Col 1-2: Centre information.']);
    push(['Col 3-7: Batch info — name, code, status, student count, expected students.']);
    push(['Col 8-13: Plan assigned to the batch — name, start date, current day, total days, progress %, behind-flags. Behind = started >=3 days ago but <40% through plan.']);
    push([]);
    push([
    ...
    push([]);
    push(['Each row = one batch assigned to you. Student-level data: use the Students page export.']);

    const blob = new Blob(['\uFEFF' + rows.join('\n')], { type: 'text/csv;charset=utf-8;' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `${center?.name || 'faculty'}_dashboard_${new Date().toISOString().split('T')[0]}.csv`;
    a.click();
  ...
  return (
    <div style={{ padding: 'var(--space-lg)', maxWidth: 1300, margin: '0 auto' }}>
      <Helmet><title>Dashboard — {center?.name || 'Faculty'} — TheWebytes</title></Helmet>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 'var(--space-md)', marginBottom: 'var(--space-lg)' }}>
        <div>
          <h1 style={{ fontSize: 'clamp(1.3rem, 3vw, 1.8rem)', fontWeight: 900, display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            Welcome back{authUser?.displayName || authUser?.username ? `, ${authUser.displayName || authUser.username}` : ''}
          </h1>
          {center && (
      ...
      {/* ═══ EMPTY STATE — no batches assigned ═══ */}
      {batches.length === 0 && (
        <div style={{ ...CARD, textAlign: 'center', padding: 'var(--space-2xl)' }}>
          <Layers size={40} style={{ color: 'var(--text-tertiary)', marginBottom: 12 }} />
          <h3 style={{ fontSize: '1.1rem', fontWeight: 900, marginBottom: 6 }}>No batches assigned yet</h3>
          <p style={{ color: 'var(--text-tertiary)', maxWidth: 420, margin: '0 auto var(--space-md)', lineHeight: 1.6 }}>
            Your centre coordinator or admin will assign batches to you. Once assigned, student progress, plans, and assignments will appear here.
          </p>
          <Link to="/faculty/students" style={{ fontSize: '0.72rem', fontWeight: 700, padding: '6px 14px', border: '3px solid #000', boxShadow: '3px 3px 0 #000', textDecoration: 'none', color: 'inherit', display: 'inline-flex', alignItems: 'center', gap: 4 }}>
            <Users size={14} /> View My Students
          </Link>
        </div>
      )}
      ...
      {batches.length > 0 && (
        <>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 12, marginBottom: 'var(--space-xl)' }}>
            <Link to="/faculty/students" style={{ textDecoration: 'none', color: 'inherit', ...CARD, padding: 'var(--space-md)', textAlign: 'center', transition: 'transform 0.12s' }}
              ...
              <div style={{ fontSize: '0.6rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.06em' }}>My Students</div>
            </Link>
            <Link to="/faculty/batches" style={{ textDecoration: 'none', color: 'inherit', ...CARD, padding: 'var(--space-md)', textAlign: 'center', transition: 'transform 0.12s' }}
              onMouseEnter={e => { e.currentTarget.style.transform = 'translate(-2px, -2px)'; e.currentTarget.style.boxShadow = '8px 8px 0 #000'; }}
              onMouseLeave={e => { e.currentTarget.style.transform = 'none'; e.currentTarget.style.boxShadow = '6px 6px 0 #000'; }}>
            ...
            <Link to="/faculty/plans" style={{ textDecoration: 'none', color: 'inherit', ...CARD, padding: 'var(--space-md)', textAlign: 'center', transition: 'transform 0.12s' }}
              onMouseEnter={e => { e.currentTarget.style.transform = 'translate(-2px, -2px)'; e.currentTarget.style.boxShadow = '8px 8px 0 #000'; }}
              onMouseLeave={e => { e.currentTarget.style.transform = 'none'; e.currentTarget.style.boxShadow = '6px 6px 0 #000'; }}>
            ...
            {stats?.aggregate?.overall?.percentage !== undefined && (
              <div style={{ ...CARD, padding: 'var(--space-md)', textAlign: 'center' }}>
                ...
                <p style={{ fontSize: '0.65rem', color: 'var(--text-tertiary)', marginTop: 4, lineHeight: 1.3, borderTop: '2px solid #000', paddingTop: 4 }}>
                  % of all lessons, topics, and problems completed across all subjects by my students. Not tied to any plan.
                </p>
              </div>
            )}
          </div>
              ...
              {batchSubjectData.length > 0 ? (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  {batchSubjectData.map(b => (
                    <Link key={b.name} to={`/faculty/batches/${b._id}`} style={{ textDecoration: 'none', color: 'inherit' }}
          ...
          <div style={{ marginBottom: 'var(--space-xl)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 'var(--space-sm)', marginBottom: 'var(--space-md)' }}>
              ...
              <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                ...
                <Link to="/faculty/batches" style={{ fontSize: '0.65rem', fontWeight: 700, padding: '4px 10px', border: '3px solid #000', boxShadow: '3px 3px 0 #000', textDecoration: 'none', color: 'inherit', display: 'inline-flex', alignItems: 'center', gap: 4 }}>
            ...
            <div style={{ ...CARD, marginBottom: 'var(--space-xl)', borderLeft: '6px solid #2563eb' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 'var(--space-sm)', marginBottom: 'var(--space-md)' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                  ...
                  <Link to="/faculty/students" style={{ fontSize: '0.65rem', fontWeight: 700, padding: '4px 10px', border: '2px solid #000', textDecoration: 'none', color: 'inherit', display: 'inline-flex', alignItems: 'center', gap: 4 }}>
                  ...
                  {paginatedTop.map(s => (
                    <Link key={s._id} to={`/faculty/students/${s._id}`} style={{
                      ...
                      <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
                        <span style={{ fontSize: '0.5rem', fontWeight: 700, textTransform: 'uppercase', padding: '2px 6px', border: '2px solid #000', background: 'var(--success-bg)', color: 'var(--success-text)' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 'var(--space-md)' }}>
            <div style={{ ...CARD }}>
              ...
              <div style={{ display: 'flex', gap: 'var(--space-sm)', flexWrap: 'wrap' }}>
                <Link to="/faculty/assignments" style={{ fontSize: '0.72rem', fontWeight: 700, padding: '6px 12px', border: '3px solid #000', boxShadow: '3px 3px 0 #000', textDecoration: 'none', color: 'inherit', display: 'inline-flex', alignItems: 'center', gap: 4 }}>
                  <FileText size={14} /> Create Assignment
                </Link>
                <Link to="/faculty/plans/new" style={{ fontSize: '0.72rem', fontWeight: 700, padding: '6px 12px', border: '3px solid #000', boxShadow: '3px 3px 0 #000', textDecoration: 'none', color: 'inherit', display: 'inline-flex', alignItems: 'center', gap: 4 }}>
                  <FileText size={14} /> Create Plan
                </Link>
                <Link to="/faculty/students" style={{ fontSize: '0.72rem', fontWeight: 700, padding: '6px 12px', border: '3px solid #000', boxShadow: '3px 3px 0 #000', textDecoration: 'none', color: 'inherit', display: 'inline-flex', alignItems: 'center', gap: 4 }}>
                  <Users size={14} /> View Students
                </Link>
                <Link to="/faculty/plans" style={{ fontSize: '0.72rem', fontWeight: 700, padding: '6px 12px', border: '3px solid #000', boxShadow: '3px 3px 0 #000', textDecoration: 'none', color: 'inherit', display: 'inline-flex', alignItems: 'center', gap: 4 }}>
            ...
            <div style={{ ...CARD }}>
              ...
              <p style={{ fontSize: '0.72rem', color: 'var(--text-tertiary)', marginBottom: 'var(--space-sm)' }}>
                Download your batch and plan data as CSV. Includes detailed column explanations. Day-by-day analysis is NOT included — visit each batch for day-level detail.
              </p>
