...
export default function AdminAptitudeList() {
  const { lessons, loading, fetchLessons, subtopics, fetchSubtopics, total, fetchProblems, deleteLesson } = useAptitudeStore();
  ...
  const rows = filteredLessons.map(l => ({
    ...
    actions: (
      <div className="admin-actions" style={{ display: 'flex', gap: '4px', flexWrap: 'wrap' }}>
        <Link to={`/admin/aptitude/lessons/${l._id}/edit`} className="btn btn--sm">Edit</Link>
        <Link to={`/admin/aptitude/lessons/${l._id}/subtopics`} className="btn btn--sm">Subtopics</Link>
  ...
  return (
    <div>
      <Helmet><title>Admin Aptitude — TheJobStarter</title></Helmet>

      <div className="admin-dsa-header">
        <div className="listing-header">
          <h1 className="listing-header__title">Aptitude Concepts</h1>
      ...
      <div className="admin-stats-grid" style={{ margin: 'var(--space-xl) 0' }}>
        <Link to="/admin/aptitude/lessons/new" className="stat-card" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '8px', padding: 'var(--space-lg)' }}>
          <BookOpen size={28} />
        ...
        <Link to="/admin/aptitude/subtopics" className="stat-card" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '8px', padding: 'var(--space-lg)' }}>
          <Layers size={28} />
        ...
        <Link to="/admin/aptitude/problems" className="stat-card" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '8px', padding: 'var(--space-lg)' }}>
          <Code2 size={28} />
      ...
      <div className="listing-header">
        <h1 className="listing-header__title">Aptitude Lessons</h1>
