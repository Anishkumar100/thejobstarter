/*
 * Seed Searching lesson content into MongoDB
 * Uses slug-based upserts — never deletes existing data.
 * Run: node dsa-content/seed_searching.mjs
 */
...
async function main() {
  ...
  const subtopics = [
    {
      slug: 'linear-and-binary-search', lessonSlug: 'searching', order: 0,
  ...
  const problems = [
    {
      slug: 'binary-search', lessonSlug: 'searching', subtopicSlug: 'linear-and-binary-search',
      title: 'Binary Search', difficulty: 'easy',
      topics: ['Searching', 'Binary Search'], companies: ['Amazon', 'Google', 'Microsoft', 'Facebook', 'Apple'],
      ...
      examples: [
        { input: 'arr = [-1, 0, 3, 5, 9, 12], target = 9', output: '4', explanation: 'Binary search trace:\n- left=0, right=5, mid=2 -> arr[2]=3 < 9 -> search right half (left=3)\n- left=3, right=5, mid=4 -> arr[4]=9 == 9 -> found! Return 4' },
        { input: 'arr = [-1, 0, 3, 5, 9, 12], target = 2', output: '-1', explanation: 'Binary search trace:\n- left=0, right=5, mid=2 -> arr[2]=3 > 2 -> search left half (right=1)\n- left=0, right=1, mid=0 -> arr[0]=-1 < 2 -> search right half (left=1)\n- left=1, right=1, mid=1 -> arr[1]=0 < 2 -> search right half (left=2)\n- left=2, right=1 -> left > right, loop exits\n- Return -1 (not found)' },
        { input: 'arr = [5], target = 5', output: '0', explanation: 'Single element: left=0, right=0, mid=0 -> arr[0]=5 == 5 -> found! Return 0' },
        { input: 'arr = [], target = 1', output: '-1', explanation: 'Empty array: left=0, right=-1 -> left > right immediately, loop never runs. Return -1.' }
      ],
      constraints: ['The array length is between 0 and 100,000 elements.', 'Each element is a 32-bit integer.', 'The array is sorted in strictly increasing order.'],
  ...
  const quizzes = [
    {
      slug: 'binary-search',
      questions: [
        { text: 'What is the one condition that must be true for binary search to work correctly?', options: ['The array must contain only positive numbers', 'The array must be sorted', 'The array must have no duplicate values', 'The array must have at least 10 elements'], correctIndex: 1 },
        { text: 'How many elements does binary search examine in the worst case when searching an array of 1,000 elements?', options: ['1,000 elements (all of them)', '500 elements (half of them)', 'About 10 elements (log2(1000) = 10)', 'Exactly 1 element'], correctIndex: 2 },
        { text: 'In the binary search while loop, why do we use `left <= right` instead of `left < right`?', options: ['They both work the same way', 'To handle the case where the array has only one element (left == right)', 'To make the loop run faster', 'To prevent integer overflow'], correctIndex: 1 },
        { text: 'What does binary search return if the target is not found in the array?', options: ['0', 'The index where the target would be inserted', '-1', 'undefined'], correctIndex: 2 },
        { text: 'Why do we write `mid = left + (right - left) / 2` instead of `mid = (left + right) / 2`?', options: ['It\'s the same formula written differently — no real difference', 'To avoid integer overflow when left and right are very large', 'To make the code run faster', 'To handle negative numbers correctly'], correctIndex: 1 }
      ]
    }
  ];
  ...
  /* ─── Done ─── */
  console.log('\n[SEED] Searching lesson seeded successfully!');
  console.log(`  Lesson:    1 (Searching)`);
  console.log(`  Subtopics: ${subtopics.length} (Linear & Binary Search)`);
  console.log(`  Problems:  ${problems.length} (Binary Search)`);
