...
/*
 * FacultyGeneralStats — General stats page for a faculty member.
 * Exact mirror of CoordinatorGeneralStats, but every endpoint, export and
 * drill-down link is scoped to the faculty member's own batches.
 */
export default function FacultyGeneralStats() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      /* The faculty students endpoint has no center field — stats carries it */
      setCenter(statsRes.data?.center || null);
  ...
  if (!center && batches.length === 0) return (
    <div style={{ padding: 'var(--space-xl)', textAlign: 'center' }}>
      <h2>No Batches Assigned</h2>
      <p style={{ color: 'var(--text-tertiary)', marginTop: 4 }}>
        You are not yet assigned to any batches. Your centre coordinator or admin will assign batches to you — stats will appear here once you have students.
      </p>
    </div>
  );
  ...
  return (
    <div style={{ padding: 'var(--space-lg)', maxWidth: 1440, margin: '0 auto' }}>
      <Helmet><title>General Stats — {center?.name || 'Faculty'} — TheWebytes</title></Helmet>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 'var(--space-sm)' }}>
          <div>
            ...
            <p style={{ fontSize: '0.82rem', color: 'var(--text-tertiary)' }}>
              {center?.name || 'Your batches'} — Overview of your students' progress and performance
            </p>
          </div>
          <button
            onClick={async () => {
              try {
                ...
                a.href = url;
                const slug = (center?.name || 'faculty').toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '');
                ...
                console.error('[FACULTY] Export error:', err.message);
              }
            }}
            style={{
      ...
      {stats && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 'var(--space-lg)', marginBottom: 'var(--space-xl)' }}>
          {[
            { icon: Users, label: 'Total Students', value: stats.totalStudents, sub: 'across your batches', color: 'var(--text-primary)' },
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(360px, 1fr))', gap: 'var(--space-xl)', marginBottom: 'var(--space-xl)' }}>
        ...
        <div style={{ ...CARD }}>
          ...
          <div style={EXPLAIN_BOX}>
            Total items (lessons + topics + problems) completed per subject by all your students. <strong>Green</strong> = done, <strong>grey</strong> = remaining. Compare subjects — a large grey section means that subject needs more focus. Allocate extra tutorials or practice to the weakest subject.
          </div>
        ...
        <div style={{ ...CARD }}>
          ...
          <div style={EXPLAIN_BOX}>
            Combined completion across <strong>all subjects</strong>. <strong>Green</strong> = done, <strong>red</strong> = remaining. As your students progress, green should grow. If red stays dominant for weeks, review the pace or investigate which subjects are dragging performance down. Best high-level indicator of cohort-wide progress.
          </div>
      ...
      <div style={{ ...CARD }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 'var(--space-md)', flexWrap: 'wrap', gap: 'var(--space-sm)' }}>
          <h2 style={{ fontSize: '0.95rem', fontWeight: 800, display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <span style={{ fontSize: '0.72rem', fontWeight: 400, color: 'var(--text-tertiary)', borderLeft: '2px solid var(--border-color)', paddingLeft: 8 }}>{students.filter(s => s.isFaculty !== true).length} total</span>
          </h2>
          <Link to="/faculty/students" className="btn btn--sm" style={{ fontSize: '0.68rem', border: '3px solid var(--border-color)', boxShadow: '3px 3px 0 var(--border-color)', display: 'inline-flex', alignItems: 'center', gap: 4, background: 'var(--accent)', color: 'var(--text-inverse)' }}>
            View All <ArrowRight size={12} />
          </Link>
        </div>
        {students.filter(s => s.isFaculty !== true).length === 0 ? (
          <div style={{ textAlign: 'center', padding: 'var(--space-lg)', border: '3px dashed var(--border-color)' }}>
            ...
            <p style={{ color: 'var(--text-tertiary)', fontSize: '0.85rem' }}>No students have joined your batches yet.</p>
          </div>
        ) : (
                ...
                {students.filter(s => s.isFaculty !== true).slice(0, 8).map((s, idx) => {
                  ...
                  return (
                    <tr key={s._id} style={{ borderBottom: '2px solid var(--border-color)', background: idx % 2 === 0 ? 'transparent' : 'var(--bg-tertiary)' }}>
                      ...
                      <td style={{ padding: '8px 12px', textAlign: 'center' }}>
                        <Link to={`/faculty/students/${s._id}`} className="btn btn--sm" style={{ fontSize: '0.65rem', padding: '4px 12px', border: '3px solid var(--border-color)', boxShadow: '2px 2px 0 var(--border-color)', fontWeight: 700, background: 'var(--accent)', color: 'var(--text-inverse)' }}>
                          View
                        </Link>
                      </td>
                    </tr>
