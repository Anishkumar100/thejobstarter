...
import {
  ...
  CheckCircle, Clock, Calendar, X, Save, Edit3, Trash2, Copy,
  Search, Plus, BarChart3, TrendingUp
} from 'lucide-react';
...
export default function AdminBatchDetail() {
  ...
  /*
   * Fetch batch and centre students
   */
  const fetchData = async () => {
    console.log('[ADMIN BATCH] Loading batch:', id);
    setLoading(true);
    setError(null);
    try {
      ...
      /* Fetch centre-scoped students for enriched progress data */
      const centerId = found.coachingCenter?._id || found.coachingCenter;
      if (centerId) {
        try {
          const rosterRes = await apiRequest(`/coaching-centers/${centerId}/students`);
          setAllStudents(rosterRes.data?.students || []);
        } catch {
          console.log('[ADMIN BATCH] Could not fetch centre roster, using un-scoped users');
          const usersRes = await apiRequest('/users');
          setAllStudents(usersRes.data || []);
        }
      } else {
        console.log('[ADMIN BATCH] No coachingCenter on batch, fetching all users');
        const usersRes = await apiRequest('/users');
        setAllStudents(usersRes.data || []);
      }
    } catch (err) {
  ...
  /* Fetch batch day progress when active plan is available */
  useEffect(() => {
  ...
  const handleSaveName = async () => {
    ...
    try {
      ...
      console.log('[ADMIN BATCH] Name updated');
    } catch (err) {
      console.error('[ADMIN BATCH] Name update error:', err.message);
  ...
  const handleSaveCode = async () => {
    ...
    try {
      ...
      console.log('[ADMIN BATCH] Code updated');
    } catch (err) {
      console.error('[ADMIN BATCH] Code update error:', err.message);
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      navigate('/admin/coaching-centers');
    } catch (err) {
      console.error('[ADMIN BATCH] Delete error:', err.message);
  ...
  /* Derive enrolled vs unassigned */
  const batchStudentIds = useMemo(() => {
    if (!batch) return [];
    return allStudents.filter(s => (s.batch?._id || s.batch) === batch._id).map(s => s._id);
  }, [allStudents, batch]);
  ...
  }, [allStudents, batchStudentIds, searchQuery, courseFilter, dateFilter]);
  ...
  const handleAssign = async (ids) => {
    ...
    setBusy(true);
    try {
      await apiRequest(`/batches/${id}/assign-students`, {
        method: 'POST',
  ...
  const handleRemoveFromBatch = async (ids) => {
    ...
    setBusy(true);
    try {
      await apiRequest(`/batches/${id}/remove-students`, {
        method: 'POST',
        body: JSON.stringify({ userIds: ids })
      });
  ...
  const handleSingleRemove = async (student) => {
    ...
    try {
      await apiRequest(`/batches/${id}/remove-student/${student._id}`, { method: 'PATCH' });
  ...
  /* Derive available subjects from actual student progress data */
  ...
  }, [enrolledStudents, availableSubjects]);

  const topCandidates = useMemo(() =>
    enrichedStudents.filter(s => s.progress?.planProgress?.paceStatus && s.progress?.planProgress?.paceStatus !== 'just-started')
      .sort((a, b) => b._pp - a._pp),
    [enrichedStudents]
  );

  const onTrackStudents = useMemo(() =>
    enrichedStudents.filter(s => s.progress?.planProgress?.paceStatus === 'on-track'),
    [enrichedStudents]
  );

  const behindStudents = useMemo(() =>
    enrichedStudents.filter(s => s.progress?.planProgress?.paceStatus === 'behind'),
    [enrichedStudents]
  );
  ...
  /* Derive unique course offerings for filter */
  const courseOptions = useMemo(() => {
    const map = {};
    allStudents.forEach(s => {
      const co = s.courseOffering;
      if (co && co._id && !map[co._id]) map[co._id] = co;
    });
    ...
    <div style={{ padding: 'var(--space-lg)', maxWidth: 1200, margin: '0 auto' }}>
        ...
        <div>
          ...
          <div style={{
            ...
            <span>
              ...
              {batch.coachingCenter?.name || 'Unknown Centre'}
            </span>
            <span>
      ...
      <div style={{ ...CARD, marginBottom: 'var(--space-lg)' }}>
        ...
        {loadingPlan ? (
          ...
          <div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap', marginBottom: 6, fontSize: '0.7rem', color: 'var(--text-tertiary)' }}>
              ...
              <span style={{ opacity: 0.55 }}>
                ({activePlan.totalDays || 0} days • Day {activePlan.currentDay ?? 0})
              </span>
            </div>
          ...
          <p style={{ fontSize: '0.82rem', color: 'var(--text-tertiary)', display: 'flex', alignItems: 'center', gap: 6 }}>
            <Clock size={14} /> No plan assigned yet. Assign a study plan to track daily progress.
          </p>
        )}
      </div>
              ...
              {paginatedTop.length === 0 ? (
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
                  {paginatedTop.map((s, idx) => {
                    const pp = s.progress?.planProgress;
                    return (
                      <Link key={s._id} to={`/admin/users/${s._id}`} style={{
                  ...
                  enrolledStudents.map(s => (
                    <tr key={s._id} style={{ borderBottom: '1px solid var(--border-subtle)' }}>
                      ...
                      <td style={{ padding: '6px 10px', fontWeight: 600, color: 'var(--text-primary)' }}>
                        <Link to={`/admin/users/${s._id}`} style={{ fontWeight: 600, textDecoration: 'none', color: 'inherit' }}
      ...
      <section>
        <h2 style={{
          ...
          <Users size={16} /> {batch.coachingCenter?.name || 'Centre'} students not in this batch ({unassignedStudents.length})
        </h2>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <h3 style={{ marginBottom: 'var(--space-md)', fontWeight: 800, color: 'var(--text-primary)' }}>
              {confirmAction.type === 'assign' ? 'Assign to Batch' : 'Remove from Batch'}
            </h3>
            ...
            <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
              ...
              <button className="btn btn--danger" onClick={executeConfirm} disabled={busy}>
                {busy ? 'Working...' : confirmAction.type === 'assign' ? 'Assign' : 'Yes, remove from batch'}
              </button>
            </div>
          </div>
        </div>
