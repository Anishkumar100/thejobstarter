...
export default function AptitudeSubtopicProblems() {
  const { lessonSlug, subtopicSlug } = useParams();
  const { problems, loading, fetchSubtopicProblems, currentSubtopic, fetchSubtopicBySlug } = useAptitudeStore();
  ...
  return (
    <div className="probs-page">
      <Helmet>
        <title>{currentSubtopic?.title || 'Problems'} — Aptitude — TheJobStarter</title>
        <meta name="description" content={`Practice problems for ${currentSubtopic?.title || subtopicSlug}`} />
      </Helmet>

      <div className="probs-hero">
        <Link to={`/aptitude/${lessonSlug}/${subtopicSlug}`} className="probs-back-link">
      ...
      <div className="probs-toolbar">
        ...
        <div className="probs-toolbar__actions">
          ...
          <Link to={`/aptitude/${lessonSlug}/${subtopicSlug}`} className="probs-toolbar__btn">
            <AiIdeaIcon size={14} />
            Study Notes
          </Link>
        </div>
