...
import { ArrowLeft, ExternalLink, TrendingUp, User, BookOpen, Database, Cpu, Target, Award, BarChart3, Brain, GraduationCap, Mail, Calendar, ClipboardList, Layers, Clock, AlertCircle, CheckCircle, FileText, Download, MessageSquare, Shield } from 'lucide-react';
import {
...
/*
 * FacultyStudentDetail — Read-only performance drill-down.
 * NO edit, NO remove buttons (faculty has no management rights).
 */
export default function FacultyStudentDetail() {
  ...
  useEffect(() => {
    if (!userId) return;
    setLoading(true);
    apiRequest(`/faculty/students/${userId}`)
      .then(res => {
        setStudent(res.data);
        setLoading(false);
      })
  ...
  useEffect(() => {
    ...
    apiRequest(`/plans/${pp.planId}/day-progress/${student.batch._id}/${userId}`)
      ...
      .catch(err => console.error('[FACULTY] Day breakdown error:', err.message))
  ...
  useEffect(() => {
    ...
    apiRequest(`/plans/batches/${student.batch._id}/plan-history`)
      ...
      .catch(err => console.error('[FACULTY] Plan history error:', err.message));
  ...
  return (
    <div style={{ padding: 'var(--space-lg)', maxWidth: 1200, margin: '0 auto' }}>
      <Helmet><title>{student.displayName || student.username} — Student — Faculty — TheWebytes</title></Helmet>

      {/* Back nav */}
      <div style={{ marginBottom: 'var(--space-md)', display: 'flex', alignItems: 'center', gap: 'var(--space-sm)', flexWrap: 'wrap' }}>
        <Link to="/faculty" style={{ display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: '0.82rem', fontWeight: 600 }}>
          <ArrowLeft size={14} /> Dashboard
        </Link>
        <span style={{ color: 'var(--text-tertiary)' }}>/</span>
        <Link to="/faculty/students" style={{ display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: '0.82rem', fontWeight: 600 }}>
          Students
        </Link>
        <span style={{ color: 'var(--text-tertiary)' }}>/</span>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-md)', flex: 1, minWidth: 250 }}>
          ...
          <div style={{ minWidth: 0 }}>
            ...
            <div style={{ display: 'flex', gap: 8, marginTop: 4, flexWrap: 'wrap' }}>
              ...
              {student.batch && (
                <Link to={`/faculty/batches/${student.batch._id}`} style={{ fontSize: '0.7rem', padding: '2px 8px', border: '2px solid var(--border-color)', background: 'var(--accent-light)', textDecoration: 'none', color: 'var(--text-primary)', fontWeight: 700 }}>
