...
export default function DsaSubtopicProblems() {
  const { lessonSlug, subtopicSlug } = useParams();
  const { problems, problemsLoading, problemsError, fetchSubtopicProblems, currentSubtopic, fetchSubtopicBySlug } = useDsaStore();
  ...
  return (
    <div className="probs-page">
      <Helmet>
        <title>{currentSubtopic?.title || 'Problems'} — DSA — TheJobStarter</title>
        <meta name="description" content={`Practice problems for ${currentSubtopic?.title || subtopicSlug}`} />
      </Helmet>

      {/* ═════ HERO ═════ */}
      <div className="probs-hero">
        <Link to={`/dsa/${lessonSlug}/${subtopicSlug}`} className="probs-back-link">
      ...
      <div className="probs-toolbar">
        ...
        <div className="probs-toolbar__actions">
          ...
          <Link to={`/dsa/${lessonSlug}/${subtopicSlug}`} className="probs-toolbar__btn">
            <AiIdeaIcon size={14} />
            Study Notes
          </Link>
        </div>
