...
/*
 * AdminDbmsMeta — CRUD management for DBMS categories, topics, and companies
 * Mirrors AdminDsaMeta but uses /api/dbms-meta endpoint
 */
export default function AdminDbmsMeta() {
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      if (editingId) {
        await apiRequest(`/dbms-meta/${editingId}`, { method: 'PUT', body: JSON.stringify(payload) });
        showMsg('Updated!');
      } else {
        await apiRequest('/dbms-meta', { method: 'POST', body: JSON.stringify(payload) });
        showMsg('Created!');
      }
      setEditingId(null);
      await fetchItems();
    } catch (err) {
      console.error('[DBMS-META] Save failed:', err.message);
  ...
  const handleDelete = async (item) => {
    ...
    try {
      await apiRequest(`/dbms-meta/${item._id}`, { method: 'DELETE' });
      showMsg('Deleted!');
      if (editingId === item._id) setEditingId(null);
      await fetchItems();
    } catch (err) {
      console.error('[DBMS-META] Delete failed:', err.message);
      showMsg('Delete failed', 'error');
    }
  };

  const handleSeed = async () => {
    if (!confirm('Replace all DBMS categories, topics, and companies with defaults?')) return;
    setSeeding(true);
    try {
      await apiRequest('/dbms-meta/seed', { method: 'POST' });
      showMsg('Seeded with defaults!');
      await fetchItems();
    } catch (err) {
      console.error('[DBMS-META] Seed failed:', err.message);
  ...
  if (loading && items.length === 0) {
    return <Loader text="LOADING DBMS META..." />;
  }
  ...
  return (
    <div>
      <Helmet><title>DBMS Meta — Admin</title></Helmet>

      <div className="listing-header">
        <h1 className="listing-header__title">DBMS Categories, Topics &amp; Companies</h1>
        ...
        <form onSubmit={handleSave}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 80px', gap: 'var(--space-sm)', alignItems: 'end' }}>
            <div className="input-group" style={{ margin: 0 }}>
              ...
              <input className="input" value={formLabel} onChange={e => handleLabelChange(e.target.value)} placeholder="e.g. SQL" required />
            </div>
            <div className="input-group" style={{ margin: 0 }}>
              <label>Value (slug/key)</label>
              <input className="input" value={formValue} onChange={e => setFormValue(e.target.value)} placeholder="e.g. sql" required />
            </div>
            <div className="input-group" style={{ margin: 0 }}>
              <label>Order</label>
