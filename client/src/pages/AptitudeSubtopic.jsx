...
export default function AptitudeSubtopic() {
  const { lessonSlug, subtopicSlug } = useParams();
  const { currentSubtopic, loading, fetchSubtopicBySlug } = useAptitudeStore();
  ...
  return (
    <div className="subtopic-page">
      <Helmet>
        <title>{title} — Aptitude — TheJobStarter</title>
        <meta name="description" content={description || ''} />
      </Helmet>

      <div className="subtopic-back-bar">
        <Link to={`/aptitude/${lessonSlug}`} className="subtopic-back-link">
      ...
      >
        ...
        <aside className="subtopic-sidebar">
          ...
          {problems && problems.length > 0 && (
            <Link to={`/aptitude/${lessonSlug}/${subtopicSlug}/problems`} className="subtopic-sidebar__cta">
