...
export default function AdminDbmsSubtopicEdit() {
  const { lessonId, id } = useParams();
  const navigate = useNavigate();
  const { lessons, fetchLessons, createSubtopic, updateSubtopic } = useDbmsStore();
  const isNew = !id;
  ...
  /* For edit mode, we fetch existing and find it */
  const { subtopics, fetchSubtopics } = useDbmsStore();
      ...
      reader.onload = async () => {
        try {
          const res = await apiRequest('/media/upload', {
            method: 'POST',
            body: JSON.stringify({ file: reader.result, fileName: `dbms-subtopic-${slug || 'new'}-${Date.now()}` })
          });
          setImage(res.url);
        } catch (err) {
  ...
  /*
   * Save the subtopic — create or update depending on isNew
   */
  const handleSave = async (e) => {
    ...
    try {
      ...
      setSaved(true);
      setTimeout(() => navigate(`/admin/dbms/lessons/${lessonId}/subtopics`), 1500);
    } catch (err) {
