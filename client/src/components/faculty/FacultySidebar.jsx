...
import {
  ...
  Building2, MapPin, Phone, Mail, Circle, Layers, Sun, Moon, GraduationCap, BarChart3
} from 'lucide-react';
import { useState } from 'react';
import { useThemeStore } from '../../stores/useThemeStore.js';

/* ── Section data (view-only scope — no centre profile page) ── */
const SECTIONS = [
  {
    heading: 'Overview',
    links: [
      { to: '/faculty', label: 'Dashboard', icon: LayoutDashboard },
      { to: '/faculty/general-stats', label: 'General Stats', icon: BarChart3 },
    ]
  },
  {
    heading: 'Students',
  ...
  },
  {
    heading: 'Batches',
    links: [
      { to: '/faculty/batches', label: 'My Batches', icon: Layers },
    ]
  },
  {
    heading: 'Assignments',
    links: [
      { to: '/faculty/assignments', label: 'Manage Assignments', icon: FileText },
    ]
  },
  {
    heading: 'Plans',
    links: [
      { to: '/faculty/plans', label: 'Study Plans', icon: FileText },
    ]
  }
];

export default function FacultySidebar({ isOpen, onToggle, center, facultyName, facultyAvatar, facultyEmail }) {
  ...
  return (
    <>
      ...
      >
        ...
        {/* ═══ THEME TOGGLE + FACULTY IDENTITY (compact row) ═══ */}
        <div style={{
          ...
          {facultyAvatar ? (
            <img src={facultyAvatar} alt="" style={{
              width: 30, height: 30, border: '2px solid #000', objectFit: 'cover', flexShrink: 0
            }} title={facultyName} />
          ) : (
            <div style={{
              width: 30, height: 30, border: '2px solid #000', background: 'var(--bg-tertiary)',
              display: 'flex', alignItems: 'center', justifyContent: 'center',
