...
export default function AdminAllSubtopics() {
  ...
  /* Fetch subtopics — with lesson filter if selected, or all if empty */
  useEffect(() => {
    const filters = {};
  ...
  const rows = subtopics.map(s => {
    ...
    return {
      ...s,
      lesson: lesson ? (
        <Link to={`/admin/dsa/lessons/${lesson._id}/subtopics`} style={{ color: 'var(--accent)' }}>
          {lesson.title}
        </Link>
      ) : s.lessonSlug,
      actions: (
        <div className="admin-actions">
          <Link to={`/admin/dsa/problems/new?lesson=${s.lessonSlug || ''}&subtopic=${s.slug}`} className="btn btn--sm">+ Problem</Link>
          <Link to={`/admin/dsa/lessons/${lesson?._id || ''}/subtopics/${s._id}/edit`} className="btn btn--sm">Edit</Link>
