...
/*
 * OsSubtopicProblems — lists all problems for a given OS subtopic.
 * Supports difficulty filtering and displays problem cards with stats.
 */
export default function OsSubtopicProblems() {
  ...
  return (
    <div className="probs-page">
      <Helmet>
        <title>{currentSubtopic?.title || 'Problems'} — OS — TheJobStarter</title>
        <meta name="description" content={`Practice problems for ${currentSubtopic?.title || subtopicSlug}`} />
      </Helmet>

      {/* ═════ HERO ═════ */}
      <div className="probs-hero">
        <Link to={`/os/${lessonSlug}/${subtopicSlug}`} className="probs-back-link">
      ...
      <div className="probs-toolbar">
        ...
        <div className="probs-toolbar__actions">
          ...
          <Link to={`/os/${lessonSlug}/${subtopicSlug}`} className="probs-toolbar__btn">
            <AiIdeaIcon size={14} />
            Study Notes
          </Link>
        </div>
        ...
        {!problemsLoading && !problemsError && filtered.length > 0 && (
          <div className="probs-grid">
            {filtered.map((problem, i) => (
              ...
              >
                <Link
                  to={`/os/${lessonSlug}/${subtopicSlug}/${problem.slug}`}
                  className="prob-card"
                >
