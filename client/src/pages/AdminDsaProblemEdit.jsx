...
export default function AdminDsaProblemEdit() {
  const { id } = useParams();
  ...
  const { topics: metaTopics, companies: metaCompanies, fetchAllMeta } = useMetaStore();
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      setSaved(true);
      setTimeout(() => navigate('/admin/dsa/problems'), 1500);
    } catch (err) {
  ...
  return (
    <div>
      ...
      <nav className="admin-breadcrumb" style={{ marginBottom: 'var(--space-md)' }}>
        ...
        <Link to="/admin/dsa/problems">Problems</Link>
        <span className="admin-breadcrumb__sep">/</span>
      ...
      <form onSubmit={handleSave} className="admin-form">
        ...
        <div className="input-group" style={{ marginTop: 'var(--space-md)' }}>
          ...
          {problemStatement && (
            <div style={{ marginTop: 'var(--space-sm)', border: '3px solid var(--border-color)', padding: 'var(--space-md)', background: 'var(--bg-surface)' }}>
              ...
              <MarkdownRenderer noAutoBullet content={problemStatement} />
            </div>
          )}
        </div>
          ...
          {examples.map((ex, i) => (
            <div key={i} className="admin-form__example-row">
              <div className="admin-form__example-io">
                <textarea className="input textarea--lg" rows={8} placeholder="Input (markdown tables supported)" value={ex.input} onChange={e => handleExample(i, 'input', e.target.value)} autoCapitalize="off" autoCorrect="off" autoComplete="off" spellCheck={false} />
                <textarea className="input textarea--lg" rows={8} placeholder="Output (markdown tables supported)" value={ex.output} onChange={e => handleExample(i, 'output', e.target.value)} autoCapitalize="off" autoCorrect="off" autoComplete="off" spellCheck={false} />
              </div>
        ...
        <div className="input-group">
          ...
          {constraints.map((c, i) => (
            <input key={i} className="input" style={{ marginBottom: '4px' }} value={c} onChange={e => handleConstraint(i, e.target.value)} placeholder="e.g., 1 <= n <= 10^5" />
          ))}
        ...
        {!isNew && <QuizEditor problemId={id} problemModel="Problem" />}
