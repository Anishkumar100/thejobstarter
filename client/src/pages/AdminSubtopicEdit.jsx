...
export default function AdminSubtopicEdit() {
  const { lessonId, id } = useParams();
  const navigate = useNavigate();
  const { lessons, fetchLessons, createSubtopic, updateSubtopic } = useDsaStore();
  const isNew = !id;
  ...
  const { subtopics, fetchSubtopics } = useDsaStore();
      ...
      reader.onload = async () => {
        try {
          const res = await apiRequest('/media/upload', {
            method: 'POST',
            body: JSON.stringify({ file: reader.result, fileName: `subtopic-${slug || 'new'}-${Date.now()}` })
          });
          setImage(res.url);
        } catch (err) {
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      setSaved(true);
      setTimeout(() => navigate(`/admin/dsa/lessons/${lessonId}/subtopics`), 1500);
    } catch (err) {
