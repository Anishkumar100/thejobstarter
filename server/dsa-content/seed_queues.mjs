/*
 * Seed Queues lesson content into MongoDB
 * Uses slug-based upserts — never deletes existing data.
 * Run: node dsa-content/seed_queues.mjs
...
async function main() {
  ...
  const subtopics = [
{
      slug: "queue-basics", lessonSlug: "queues", order: 0,
      title: "Queue Basics",
...
{
      slug: "deque", lessonSlug: "queues", order: 1,
      title: "Deque",
  ...
  const problems = [
{
      slug: "implement-queue-using-stacks", lessonSlug: "queues", subtopicSlug: "queue-basics",
      ...
      examples: [{"input":"push(1), push(2), push(3), pop()","output":"1","explanation":"Elements enter in order 1, 2, 3 — the first one in must be the first one out, so pop returns 1."},{"input":"push(1), push(2), peek(), pop(), empty()","output":"1, 1, false","explanation":"peek sees the front (1); pop removes it; the queue still holds 2 so it is not empty."},{"input":"push(1), pop(), empty()","output":"1, true","explanation":"After pushing and popping the only element, the queue is empty."},{"input":"push(1), push(2), push(3), pop(), peek()","output":"1, 2","explanation":"pop removes 1; the new front is 2 so peek returns 2."}],
      constraints: ["At most 100 operations will be called.","pop and peek are only called when the queue is non-empty.","All values pushed are integers."],
    ...
    },
{
      ...
      examples: [{"input":"nums = [1, 3, -1, -3, 5, 3, 6, 7], k = 3","output":"[3, 3, 5, 5, 6, 7]","explanation":"Each window's maximum: [1,3,-1]->3, [3,-1,-3]->3, [-1,-3,5]->5, [-3,5,3]->5, [5,3,6]->6, [3,6,7]->7."},{"input":"nums = [1], k = 1","output":"[1]","explanation":"A single window of size 1 contains just 1, so its max is 1."},{"input":"nums = [1, -1], k = 1","output":"[1, -1]","explanation":"Every window is a single element, so the maxima are the elements themselves."},{"input":"nums = [9, 11], k = 2","output":"[11]","explanation":"One window [9, 11] — the maximum is 11."}],
      constraints: ["The array length is between 1 and 100,000.","k is between 1 and the array length.","Each element is an integer between -10,000 and 10,000."],
  ...
  const quizzes = [
{
      ...
      questions: [{"text":"What order does a queue follow?","options":["Last-In-First-Out (LIFO)","First-In-First-Out (FIFO)","Random order","Sorted order"],"correctIndex":1},{"text":"In the two-stack queue, where does push(x) put the element?","options":["Onto the 'out' stack","Onto the 'in' stack","Into the middle of both stacks","Nowhere — queues don't use stacks"],"correctIndex":1},{"text":"When is the 'out' stack refilled from the 'in' stack?","options":["On every operation","Only when the 'out' stack is empty","Only when the 'in' stack is empty","When the queue is full"],"correctIndex":1},{"text":"What is the amortised time complexity of each queue operation?","options":["O(n)","O(log n)","O(n^2)","O(1)"],"correctIndex":3},{"text":"After push(1), push(2), push(3), what does pop() return?","options":["3","2","1","Nothing — the queue is empty"],"correctIndex":2}]
    },
{
      slug: "sliding-window-maximum",
      questions: [{"text":"What kind of deque does Sliding Window Maximum keep?","options":["A monotonic increasing deque","A monotonic decreasing deque","A random deque","No deque at all"],"correctIndex":1},{"text":"Where is the current window's maximum stored?","options":["At the back of the deque","At the middle of the deque","At the front of the deque","Nowhere — it is recomputed each time"],"correctIndex":2},{"text":"When do we remove an index from the front of the deque?","options":["When it is smaller than the new element","When it has fallen out of the current window","When it is bigger than the new element","Never"],"correctIndex":1},{"text":"Why do we pop smaller-or-equal values from the back before adding the new element?","options":["To sort the array","Because they can never be a window maximum while the newer bigger value is present","To save memory","To keep the deque empty"],"correctIndex":1},{"text":"What is the time complexity of the sliding window maximum solution?","options":["O(n * k)","O(n^2)","O(n log n)","O(n)"],"correctIndex":3}]
    }
  ];
  ...
  /* ─── Done ─── */
  console.log('\n[SEED] Queues lesson seeded successfully!');
  console.log('  Lesson:    1 (Queues)');
